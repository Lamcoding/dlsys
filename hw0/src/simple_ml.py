...
def softmax_loss(Z, y):
    ...
    """
    ### BEGIN YOUR CODE
    return 1.0/Z.shape[0]*np.sum(np.log(np.sum(np.exp(Z),1))-Z[np.arange(Z.shape[0]).T,y])
    ### END YOUR CODE
...
def softmax_regression_epoch(X, y, theta, lr = 0.1, batch=100):
    ...
    """
    ### BEGIN YOUR CODE
    for batchid in range(math.ceil(X.shape[0]/batch)):
        ...
        Z = np.exp(batchX @ theta)
        Z = Z / np.expand_dims(np.sum(Z,1),axis=1)
        grad = 1.0/batch_size*(batchX.T@(Z-np.eye(theta.shape[1])[batchy]))
        theta -= lr*grad
    ### END YOUR CODE
...
def nn_epoch(X, y, W1, W2, lr = 0.1, batch=100):
    ...
    for batchid in range(math.ceil(X.shape[0]/batch)):
      ...
      batchy = y[batchid*batch:min((batchid+1)*batch,y.shape[0])]
      batch_size = batchX.shape[0]
      Z1 = np.maximum(batchX @ W1,0)
      temp = np.exp(Z1 @ W2)
      temp = temp / np.expand_dims(np.sum(temp,1),axis=1)
      G2 = temp - np.eye(W2.shape[1])[batchy]
      G1 = (Z1>0) * (G2 @ W2.T)
      grad1 = 1/batch_size * batchX.T @ G1
      grad2 = 1/batch_size * Z1.T @ G2
      W1 -= lr * grad1
      W2 -= lr * grad2
    ### END YOUR CODE
