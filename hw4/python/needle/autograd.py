...
### Not needed in HW1, but needed in HW4
...
class Tensor(Value):
    ...
    def max(self, axes=None):
        return needle.ops.Amax(axes)(self)
...
def compute_gradient_of_variables(output_tensor, out_grad):
    ...
    for i in reverse_topo_order:
        ...
        if not i.is_leaf():
            inputs_fakegrad = i.op.gradient_as_tuple(i.grad, i)
            for j,k in enumerate(i.inputs):
                if k not in node_to_output_grads_list.keys():
                    node_to_output_grads_list[k] = []
                node_to_output_grads_list[k].append(inputs_fakegrad[j])
    ### END YOUR SOLUTION
...
def topo_sort_dfs(node, visited, topo_order):
    ...
    if node.is_leaf():
        ...
        return
    # topo_sort_dfs(node.inputs[0],visited,topo_order)
    # if len(node.inputs)==2:
    #     topo_sort_dfs(node.inputs[1],visited,topo_order)
