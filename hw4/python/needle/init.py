...
def one_hot(n, i, device=None, dtype="float32", requires_grad=False):
    ...
    device = ndl.cpu() if device is None else device
    return ndl.Tensor(device.one_hot(n,i.numpy().astype("int32"), dtype=dtype), device=device, requires_grad=requires_grad)
...
def xavier_uniform(fan_in, fan_out, shape=None, gain=1.0, **kwargs):
    ...
    else: return rand(*shape,low=-a,high=a,**kwargs)
    ### END YOUR SOLUTION
...
def xavier_normal(fan_in, fan_out, shape=None, gain=1.0, **kwargs):
    ...
    else: return randn(*shape,mean=0.,std=std,**kwargs)
    ### END YOUR SOLUTION
...
def kaiming_uniform(fan_in, fan_out, shape=None, nonlinearity="relu", **kwargs):
    ...
    else: return rand(*shape,low=-bound,high=bound,**kwargs)
    ### END YOUR SOLUTION
