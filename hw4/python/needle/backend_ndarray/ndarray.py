...
class NDArray:
    ...
    def broadcast_to(self, new_shape):
        ...
        """
        ### BEGIN YOUR SOLUTION
        assert all([new_shape[i] == self.shape[i] for i in range(len(self.shape)) if self.shape[i] != 1])
        new_strides = tuple(0 if self.shape[i]==1 else self.strides[i] for i in range(len(self.shape)))
        return self.as_strided(new_shape,new_strides)
        ### END YOUR SOLUTION
    ...
    def __getitem__(self, idxs):
        ...
        new_strides = tuple(idxs[i].step * self.strides[i] for i in range(len(self.strides)))
        new_offset = sum(idxs[i].start * self.strides[i] for i in range(len(self.strides)))
        return NDArray.make(shape=new_shape,strides=new_strides,device=self.device,handle=self._handle,offset=new_offset)
        ### END YOUR SOLUTION
    ...
    def sum(self, axes=None, keepdims=False):
        if axes is None:
            ...
            for axis in axes:
                ...
                a.device.reduce_sum(view.compact()._handle, out._handle, view.shape[-1])
                a = out
            return a
    ...
    def flip(self, axes):
        ...
        cp_strides = list(NDArray.compact_strides(self.shape))
        offset = 0
        for axe in axes:
            offset += (self.shape[axe]-1) * cp_strides[axe]
            cp_strides[axe] *= -1
        ret = NDArray.make(shape=self.shape,strides=tuple(cp_strides),device=self.device,handle=self._handle,offset=offset)
        return ret.compact()
        ### END YOUR SOLUTION
    ...
    def pad(self, axes):
        ...
        for i, axe in enumerate(axes):
            newshape.append(self.shape[i]+axe[0]+axe[1])
        ret = NDArray.make(newshape,device=self.device)
        ret.fill(0)
        # calculate the slices
        slices = []
        for i, axe in enumerate(axes):
            slices.append(slice(axe[0],newshape[i]-axe[1],None))
...
def summation(a, axes=None, keepdims=False):
    return a.sum(axes=axes, keepdims=keepdims)
